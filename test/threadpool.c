...
struct ThreadPool 
{
	...
	pthread_mutex_t mutexBusy;		// 锁busyNum变量,因为它的变化比较多
...
void threadPoolAdd(ThreadPool * pool, void(*func)(void*), void* arg)
{
	...
	pthread_cond_signal(&pool->notEmpty); // 队列有任务,唤醒消费者
	pthread_mutex_unlock(&pool->mutexPool);
...
void* worker(void* arg)
{
	...
	while (1)
	{
		...
		pool->queueSize--;
		// 解锁
		pthread_cond_signal(&pool->notFull); // 队列无任务,唤醒生产者,与上面遥相呼应
        pthread_mutex_unlock(&pool->mutexPool);
		// 调用线程函数
		// 开始工作
